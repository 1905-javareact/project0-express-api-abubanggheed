...
router.get('/status/:statusId', [checkRole('finance-manager'), async (req, res) => {
  try {
    ...
    const reimbursements = await getReimbursmentsByStatusService(req.params.statusId, start || '1200-01-01', end || 'now()', limit || 100, offset || 0)
...
router.get('/author/userId/:id', [checkRoleAndId('finance-manager'), async (req, res) => {
  try {
    ...
    const reimbursements = await getReimbursmentsByUserIdService(req.params.id, start || '1200-01-01', end || 'now()', limit || 100, offset || 0)
    res.json(reimbursements)
  } catch (error) {
    switch (error) {
      default:
